...
def get_items_from_file(filename):
    ...
    for item in items:
        ...
        #     'building': 'Корпус'
        # }
        #TODO У Дом ру скорее всего будет Технология подключения клиента, постараюсь предоставить обратную связь завтра.
        internet_tech = {
            'Технология подключения клиента': item['Технология подключения клиента'],
            'Технология подключения дома': item['Технология подключения дома']
        }
        if internet_tech:
            line = {
                'fias': item['Код ФИАС'],
                'locality': item['Нас. пункт'],
                'street': item['Название улицы'],
                'house': item['Дом'],
...
def get_rows_from_db(database, lines, provider_name):
    column_names = ['fias', 'locality', 'street', 'house', 'house_building']
...
def main():
    filename = f'../files_csv/Дом ру.csv'
    ...
    updates = get_rows_from_db(database, lines, 'dom-ru')
